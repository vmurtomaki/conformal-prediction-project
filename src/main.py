...
def initialize_system():
    ...
    cached_mapie = calibrate_mapie_model(
        base_model, X_train, y_train, n_blocks=CONFIG["bootstrap_estimators"]
    )
...
def main() -> None:
    ...
    with st.spinner("Processing dynamic temporal equations..."):
        ...
        # step_size kept small relative to the ACI paper's per-timestep update:
        # the engine now adapts alpha every observation internally, so this only
        # controls prediction/partial_fit batching granularity, not ACI reactivity.
